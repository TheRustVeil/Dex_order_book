import { Order, OrderSide, OrderType, PriceLevel, OrderBookSnapshot, Trade } from '@/types'
...
export class OrderBook {
  ...
  private match(taker: Order): Trade[] {
    ...
    for (const price of prices) {
      ...
      for (let i = 0; i < makers.length; i++) {
        ...
        if (taker.filled >= taker.quantity) break
        if (maker.status === 'cancelled') continue
  ...
  getTraderOrders(trader: string, status?: string): Order[] {
    const all = Array.from(this.orders.values()).filter(o => o.trader === trader)
