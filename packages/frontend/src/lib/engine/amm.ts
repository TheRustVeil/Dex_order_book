import { AMMPool, LPPosition, ILResult } from '@/types'
...
export class AMMEngine {
  ...
  addLiquidity(amountA: number, amountB: number): {
    ...
    const sharePercent = this.pool.totalLiquidity > 0
      ? (sharesIssued / this.pool.totalLiquidity) * 100
      : 100
    const position: LPPosition = {
      shares: sharesIssued,
      lpBalance: sharesIssued,
      totalSupply: this.pool.totalLiquidity,
      sharePercent,
      valueA: actualAmountA,
      valueB: actualAmountB,
  ...
  calcImpermanentLoss(position: LPPosition): ILResult {
    ...
    const priceRatio = currentPrice / (position.entryPriceAinB ?? currentPrice)

    // IL formula: IL = 2*sqrt(r)/(1+r) - 1  where r = price ratio
    const ilFactor = (2 * Math.sqrt(priceRatio)) / (1 + priceRatio) - 1

    // Value if held (no LP)
    const holdValueA = (position.entryReserveA ?? 0) * currentPrice
    const holdValueB = position.entryReserveB ?? 0
